...
class PresentController: UIViewController,NSFetchedResultsControllerDelegate {
...
//        createline(view: item)
    ...
    func createstarlabel(view:UIView){
        ...
        label.text="送了1个星星"
        view.addSubview(label)
        createstarimg(view: view)
    }
    func creatediomendlabel(view:UIView){
        let label=UILabel(frame: CGRect(x: 33, y: 72, width: 150, height: 30))
        label.backgroundColor=UIColor.clear
        label.textAlignment = .left
        label.font=UIFont.systemFont(ofSize: 18)
        label.text="送了1个钻石"
        view.addSubview(label)
        creatediomendimg(view: view)
    }
    func createstarimg(view:UIView){
        let img=UIImageView(image: UIImage(named: "组 1234"))
        img.frame=CGRect(x: 136, y: 76, width: 20, height: 20)
        view.addSubview(img)
    }
    func creatediomendimg(view:UIView){
        let img=UIImageView(image: UIImage(named: "组 1235"))
        img.frame=CGRect(x: 136, y: 76, width: 20, height: 20)
        view.addSubview(img)
    }
    /*
...
//        改变按钮的状态
    ...
    */
    ...
    func impliment_remark_list(){
        print("count: \(self.list_of_gift.count)")
        for record in self.list_of_gift{
            let phoneNum = record.from_phoneNum
...
//    渲染视图
    func setupUI(){
        print("setupUI>>>>>>>>>>>>>>>>>>>>>\(self.list_of_gift.count)")
        for record in self.list_of_gift{
            print(record.from_user?.avatar!)
            print(record.from_user?.nickname!)
            print(record.starOrDiamond)
            print(record.number)
//MARK: todo
        }
    }
