...
class WeekHistoryController: UIViewController {
...
//        为屏幕添加右滑动事件
    ...
    @objc func tonext(){
        ...
        .instantiateViewController(withIdentifier: "allhistoryboard") as UIViewController;
...
//    获取周训练数据
    func get_week_train_data(){
        week_train_history.removeAll()
        var day_set: Set<String> = Set<String>()
//          最近一周构成的集合
        for i in 0...6{
            day_set.insert(get_day_before(of_day_num: i))
        }
        for train in all_train_history{
            if day_set.contains(train.trainTime!){
                week_train_history.append(train)
            }
        }
        print("周训练次数： \(week_train_history.count)")
    }
}
