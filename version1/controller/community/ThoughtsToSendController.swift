//
//  ThoughtsToSendController.swift
//  version1
//
...
class ThoughtsToSendController: UIViewController, UITextViewDelegate, UIImagePickerControllerDelegate & UINavigationControllerDelegate,NSFetchedResultsControllerDelegate{
...
//    一次发帖记录
    var post = PostEntity()
//    录音机
    var audio_proc: audioProcessing?
//    记录时间的录音时长定时器
...
//    添加录音图标
    @IBOutlet weak var audio_label: UILabel!
    ...
    override func viewDidLoad() {
        ...
        recorder.addTarget(self, action: #selector(addAudioRecord), for: .touchUpInside)
...
//        初始化音频处理器
        self.audio_proc = audioProcessing.init()
//        设置音频的文件名称（音频名称加上扩展名）
        self.post.audio_path = (self.audio_proc?.audio_path)!
        
        
        print("button_group_size: \(button_group.count)")
...
//    发表动态按钮被点击
    ...
    @objc func addAudioRecord(){
//        正在录音状态
        if recorderState{
//            停止录音
            audio_proc?.stopRecording()
            self.audio_timer?.invalidate()
            self.audio_timer = nil
            self.audio_label.text = "录音成功"
             recorder.setBackgroundImage(UIImage(named: "组 1230"), for: .normal)
            recorderState = false
        }else {
//            未录音状态
            audio_proc?.startRecording()
//            重新开始录音，录音的时长设置为初始值0
            self.total_audio_time = 0
            self.audio_timer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(update_audio_time), userInfo: nil, repeats: true)
             recorder.setBackgroundImage(UIImage(named: "组 1232"), for: .normal)
            recorderState = true
        }
        recorder.contentMode = .scaleToFill
    }
//    更新录音时长
