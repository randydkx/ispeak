...
class ZHFSegmentVC: UIViewController {
    ...
    var titleColor = ZHFColor.red //title默认颜色
    var titleSelectedColor = ZHFColor.red //title选中颜色
    var titleScale :CGFloat = 1.4 // title有缩放效果，选中时是没选中的1.3倍
    ...
    func refreshFrame() {
        ...
        if isHave_Tabbar == true {
          UIView.animate(withDuration: 1, animations: {
            self.contentScrollView.frame = CGRect(x: 0, y: y, width: ScreenWidth, height: ScreenHeight - y - 50)
          })
//            contentScrollView.frame = CGRect(x: 0, y: y, width: ScreenWidth, height: ScreenHeight - y - 50)
        }
        else{
          UIView.animate(withDuration: 1, animations: {
            self.contentScrollView.frame = CGRect(x: 0, y: y, width: ScreenWidth, height: ScreenHeight - y)
          })
//            contentScrollView.frame = CGRect(x: 0, y: y, width: ScreenWidth, height: ScreenHeight - y)
        }
        titleViewBottomLine.frame = CGRect(x: 0, y: titleScrollView.frame.maxY, width: ScreenWidth, height: 1)
    }
    //设置按钮下划线
    func setTitleBtnBottomLine(){
//        设置按钮的下划线颜色
        titleBtnBottomLine.backgroundColor = ZHFColor.init(red: 33.0/255, green: 155.0/255, blue: 87.0/255, alpha: 1)
    ...
    func setAngle(){
        for i in 0 ..< self.titleBtns.count {
            ...
            if pointArr.count > i{
                ...
                let titleLenth: CGFloat = CGFloat(32)
...
//        self.contentScrollView.contentOffset = CGPoint(x: x, y: 0);
    ...
    func selButton(btn: UIButton){
        ...
        selectId = btn.tag
        //移动线
        let x1 :CGFloat = radioBtn.center.x - btnW/2
//        设置移动线的高度
        
        UIView.animate(withDuration: 0.2, animations: {
            self.titleBtnBottomLine.frame = CGRect(x: x1, y: self.titleScrollViewH - 1, width: self.btnW, height: 1)
        })
    }
//    重新设置视图，即scrollview
...
extension ZHFSegmentVC{
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //移动线
        let x1 :CGFloat = radioBtn.center.x - btnW/2 + scaleR * btnW
//        titleBtnBottomLine.frame = CGRect(x: x1, y: titleScrollViewH - 1, width: btnW, height: 1)
        UIView.animate(withDuration: 0.2, animations: {
          self.titleBtnBottomLine.frame = CGRect(x: x1, y: self.titleScrollViewH - 1, width: self.btnW, height: 1)
        })
    }
